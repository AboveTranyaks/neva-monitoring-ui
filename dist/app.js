...
function isFalseKtsOrOs(){return document.querySelector('input[name="alarmResult"]:checked')?.value==='Ложная'&&['КТС','ОС'].includes($('finishSystemType').value);}
function updateDispatchAvailability(){const disabled=isFalseKtsOrOs(),fieldset=document.querySelector('input[name="dispatchType"]').closest('fieldset'),reasonLabel=$('finishDispatchReason').closest('label');fieldset.classList.toggle('dispatch-not-required',disabled);reasonLabel.classList.toggle('dispatch-not-required',disabled);document.querySelectorAll('input[name="dispatchType"]').forEach(input=>{input.disabled=disabled;if(disabled)input.checked=false;});$('finishDispatchReason').disabled=disabled;if(disabled)$('finishDispatchReason').value='';}
$('finishBtn').addEventListener('click',()=>{if(!requireWorkingShift())return;clearFinishChoices();$('finishObjectLabel').textContent=`Объект ${selected.number} • ${selected.name}`;$('finishNote').value='';$('finishAlarmKind').value='';$('finishDispatchReason').value='';$('createServiceRequest').checked=false;$('serviceRequestFields').hidden=true;$('serviceRequestDescription').value='';$('serviceRequestPriority').value='Низкий';$('finishSystemType').value=isKts(selected)?'КТС':/пожар|пс/i.test(`${selected.event} ${selected.services}`)?'ПС':'ОС';updateDispatchAvailability();$('finishDialog').showModal();});
...
document.querySelectorAll('input[name="alarmResult"]').forEach(input=>input.addEventListener('change',updateDispatchAvailability));
$('finishSystemType').addEventListener('change',updateDispatchAvailability);
$('confirmFinishBtn').addEventListener('click',()=>{
  ...
  const noDispatchRequired=isFalseKtsOrOs(),action=actionChoice.value,result=resultChoice.value,dispatchType=noDispatchRequired?'Без дислокации':dispatchChoice?.value||'Не выбрана',alarmKind=$('finishAlarmKind').value||selected.event,dispatchReason=noDispatchRequired?'Не требуется при ложной КТС/ОС':$('finishDispatchReason').value||'Не указана',createRequest=$('createServiceRequest').checked,requestDescription=$('serviceRequestDescription').value.trim(),requestPriority=$('serviceRequestPriority').value;
