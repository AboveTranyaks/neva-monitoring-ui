...
function renderRows() {
  ...
  $('alarmRows').innerHTML = rows.map(a => {const newKts=isKts(a)&&a.status==='new';return `<tr class="${a.status} ${newKts?'kts':''} ${a.number===selected.number?'selected':''}" data-number="${a.number}" tabindex="0"><td><strong>${a.time}</strong><small>${a.number}</small></td><td>${a.name}</td><td class="address-cell"><span title="${a.address}">${a.address}</span></td><td>${a.event}</td><td><span class="status ${statusClass(a)} ${newKts?'kts':''}">${a.statusLabel}</span></td><td>${a.operator}</td><td class="timer ${a.status==='new'&&a.elapsedSec>600?'critical':''}">${formatElapsed(a.elapsedSec)}</td></tr>`;}).join('');
...
function openCallCenter(){$('contactsDialog').close();$('callCenterObjectLabel').textContent=`Объект ${selected.number} • ${selected.name}`;$('callCenterObject').value=`${selected.number}/1 — ${selected.name}`;$('callCenterTemplate').value='';$('callCenterSelectAll').checked=false;$('callCenterContacts').innerHTML=responsibleContacts.map((contact,index)=>`<label><input type="checkbox" class="call-recipient" value="${index}"> <span><b>${contact[0]}</b><small>${contact[3]}</small></span></label>`).join('');$('callCenterDialog').showModal();}
...
$('startCallCenter').addEventListener('click',()=>{const recipients=[...document.querySelectorAll('.call-recipient:checked')].map(input=>responsibleContacts[Number(input.value)][0]);const template=$('callCenterTemplate').value;if(!recipients.length){toast('Выберите хотя бы одно ответственное лицо');return;}if(!template){toast('Выберите шаблон звонка');return;}addHistory('Запущен звонок через колл-центр',`${template} • ${recipients.join(', ')}`);$('callCenterDialog').close();toast(`Звонок запущен: ${template}`);});
