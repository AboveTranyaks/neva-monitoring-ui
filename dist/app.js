...
function callResultMarkup(alarm){if(!alarm.callResults?.length)return`<span class="status ${statusClass(alarm)}">${alarm.statusLabel}</span>`;const primary=alarm.callResults[0],type=callResultTypes[primary.type];return`<span class="call-result-trigger" tabindex="0" aria-label="${type.label}"><img src="${type.icon}" alt="${type.label}"><span class="call-result-popup"><strong>Результаты звонков</strong>${alarm.callResults.map(result=>{const item=callResultTypes[result.type];return`<span class="call-result-popup-row ${item.answered?'answered':'missed'}"><img src="${item.icon}" alt=""><span><b>${result.phone}</b><small>${result.name}</small><em>${item.label}</em></span></span>`;}).join('')}</span></span>`;}
