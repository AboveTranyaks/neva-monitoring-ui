...
function exportReportsToExcel(){const reports=getFilteredReports();if(!reports.length){toast('Нет отчётов для выгрузки');return;}const rows=[];const row=cells=>`<Row>${cells.map(value=>`<Cell><Data ss:Type="String">${xmlEscape(value)}</Data></Cell>`).join('')}</Row>`;reports.forEach((report,index)=>{if(index)rows.push(row(['']));rows.push(row([`Карточка объекта ${report.number}`,report.name]));rows.push(row(['Адрес',report.address]));rows.push(row(['Пультовый номер',report.number]));rows.push(row(['Пульт',report.panel||'—']));rows.push(row(['Раздел',report.section||'—']));rows.push(row(['Категория',report.category||2]));rows.push(row(['Событие',report.event]));rows.push(row(['Завершено',report.completedAt]));rows.push(row(['Оператор',report.operator]));rows.push(row(['Результат',report.result]));rows.push(row(['Комментарий оператора',report.operatorComment||'Комментарий не добавлен']));rows.push(row(['ГБР',report.gbr]));rows.push(row(['Администратор',report.administrator]));rows.push(row(['Менеджер',report.manager]));rows.push(row(['Хронология']));rows.push(row(['Время','Действие','Оператор','Детали']));sortHistory(report.chronology,'asc').forEach(item=>rows.push(row([item.time,item.event,item.operator,item.details||'—'])));});const xml=`<?xml version="1.0"?><Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet"><Worksheet ss:Name="Завершённые тревоги"><Table>${rows.join('')}</Table></Worksheet></Workbook>`;const url=URL.createObjectURL(new Blob(['\ufeff',xml],{type:'application/vnd.ms-excel;charset=utf-8'}));const link=document.createElement('a');link.href=url;link.download=`Отчёты_по_тревогам_${new Date().toISOString().slice(0,10)}.xls`;link.click();setTimeout(()=>URL.revokeObjectURL(url),1000);toast('Отчёт Excel сформирован');}
