...
function renderRows() {
  ...
  $('alarmRows').innerHTML = rows.map(a => {const newKts=isKts(a)&&a.status==='new',pinned=pinnedAlarmNumbers.has(a.number),leftStatus=a.status==='new'?(newKts?'КТС':'НОВАЯ'):'—',state=a.status==='new'?`<button class="take-alarm-row" data-number="${a.number}">Взять тревогу</button>`:callResultMarkup(a);return `<tr class="${a.status} ${newKts?'kts':''} ${pinned?'pinned':''} ${a.number===selected.number?'selected':''}" data-number="${a.number}" tabindex="0"><td class="pin-cell"><button class="pin-alarm ${pinned?'active':''}" data-pin-number="${a.number}" aria-pressed="${pinned}" title="${pinned?'Открепить тревогу':'Закрепить тревогу'}">📌</button></td><td><strong>${a.time}</strong><small>${a.number}</small></td><td>${a.name}</td><td class="address-cell"><span title="${a.address}">${a.address}</span></td><td>${a.event}</td><td><span class="status ${statusClass(a)} ${newKts?'kts':''}">${leftStatus}</span></td><td class="alarm-state-cell">${state}</td><td>${a.operator}</td><td class="timer ${a.status==='new'&&a.elapsedSec>600?'critical':''}">${formatElapsed(a.elapsedSec)}</td></tr>`;}).join('');
