...
let filter = 'all';
let pinnedAlarmNumbers = new Set();
try{pinnedAlarmNumbers=new Set(JSON.parse(localStorage.getItem('neva-pinned-alarms')||'[]'));}catch(error){}
let shiftMode = 'off';
...
function sortedAlarms(list) {
  return [...list].sort((a,b) => {
    const aPinned=pinnedAlarmNumbers.has(a.number),bPinned=pinnedAlarmNumbers.has(b.number);
...
function toggleAlarmPin(number){if(pinnedAlarmNumbers.has(number))pinnedAlarmNumbers.delete(number);else pinnedAlarmNumbers.add(number);savePinnedAlarms();renderRows();toast(pinnedAlarmNumbers.has(number)?'Тревога закреплена вверху':'Тревога откреплена');}
const callResultTypes={
  arrival:{label:'Клиент прибудет на объект',instruction:'Ожидайте прибытия ответственного лица и контролируйте объект.',icon:'assets/call-arrival.png',answered:true},
  dispatch:{label:'Выставить дислокацию',instruction:'Оператору необходимо направить ближайшую ГБР.',icon:'assets/call-dispatch.png',answered:true},
  noDispatch:{label:'Дислокацию не выставлять',instruction:'Выезд ГБР не требуется. Зафиксируйте подтверждение клиента.',icon:'assets/call-no-dispatch.png',answered:true},
  noAnswer:{label:'Не дозвонились',instruction:'Повторите попытку или свяжитесь с оператором колл-центра.',icon:'assets/call-no-answer.png',answered:false},
  operator:{label:'Связаться с оператором',instruction:'Клиенту нужна связь с оператором. Продолжите обработку вручную.',icon:'assets/call-operator.png',answered:true}
};
function callResultTypeForTemplate(template){if(template==='Дислокация')return'dispatch';if(template==='Пожарка')return'arrival';if(template==='Тампер')return'noDispatch';if(template==='7-11')return'operator';if(template==='no_signal_ai')return'noAnswer';return'arrival';}
function alarmStateLabel(alarm){return alarm.statusLabel.replace(/\s+НАПРАВЛЕН(?:А|Ы)$/i,'');}
function callResultMarkup(alarm){if(!alarm.callResults?.length)return`<span class="status ${statusClass(alarm)}">${alarmStateLabel(alarm)}</span>`;const primary=alarm.callResults[0],type=callResultTypes[primary.type];return`<span class="call-result-trigger" tabindex="0" aria-label="${type.label}"><img src="${type.icon}" alt="${type.label}"><span class="call-result-popup"><strong>Результаты звонков</strong>${alarm.callResults.map(result=>{const item=callResultTypes[result.type];return`<span class="call-result-popup-row ${item.answered?'answered':'missed'}"><img src="${item.icon}" alt=""><span><b>${result.phone}</b><small>${result.name}</small><em>${item.label}</em></span></span>`;}).join('')}</span></span>`;}
...
function renderRows() {
  ...
  if(filter==='complete'){
    ...
    $('alarmRows').innerHTML=reports.map(report=>`<tr class="complete" data-report-number="${report.number}" tabindex="0"><td></td><td><strong>${report.completedAt.split(', ')[1]||'—'}</strong>${pultNumberMarkup(report)}</td><td>${report.name}</td><td class="address-cell"><span title="${report.address}">${report.address}</span></td><td>${report.event}</td><td><span class="status complete">ЗАВЕРШЕНА</span></td><td>Отчёт ›</td><td>${report.operator}</td><td>—</td></tr>`).join('');
    ...
    updateAlarmCounts();applyAlarmActionAvailability();return;
  }
  ...
  $('alarmRows').innerHTML = rows.map(a => {const newKts=isKts(a)&&a.status==='new',pinned=pinnedAlarmNumbers.has(a.number),leftStatus=a.status==='new'?(newKts?'КТС':'НОВАЯ'):'—',state=a.status==='new'?`<button class="take-alarm-row" data-number="${a.number}">Взять тревогу</button>`:callResultMarkup(a);return `<tr class="${a.status} ${newKts?'kts':''} ${pinned?'pinned':''} ${a.number===selected.number?'selected':''}" data-number="${a.number}" tabindex="0"><td class="pin-cell"><button class="pin-alarm ${pinned?'active':''}" data-pin-number="${a.number}" aria-pressed="${pinned}" title="${pinned?'Открепить тревогу':'Закрепить тревогу'}">📌</button></td><td><strong>${a.time}</strong>${pultNumberMarkup(a)}</td><td>${a.name}</td><td class="address-cell"><span title="${a.address}">${a.address}</span></td><td>${a.event}</td><td><span class="status ${statusClass(a)} ${newKts?'kts':''}">${leftStatus}</span></td><td class="alarm-state-cell">${state}</td><td>${a.operator}</td><td class="timer ${a.status==='new'&&a.elapsedSec>600?'critical':''}">${formatElapsed(a.elapsedSec)}</td></tr>`;}).join('');
  ...
  document.querySelectorAll('.pin-alarm').forEach(button=>button.addEventListener('click',e=>{e.stopPropagation();toggleAlarmPin(button.dataset.pinNumber);}));
  updateAlarmCounts();applyAlarmActionAvailability();
}
...
function renderGbr() {
  ...
  $('gbrList').innerHTML=objectGbrUnits.map((unit,index)=>{const dispatched=selected.dispatchedGbr.includes(unit.id),progress=unit.dispatchProgress||'sent',progressLabel=progress==='arrived'?'Прибыл на объект':progress==='accepted'?'Принял тревогу':'Направлена',progressInfo=progress==='arrived'?`Прибыл: ${unit.arrivedAt}`:progress==='accepted'?`Принял: ${unit.acceptedAt} • ожидается прибытие`:'Ожидается подтверждение экипажа',blocked=selected.status==='new';return `<article class="gbr-unit ${index===0?'primary':''}"><div class="gbr-unit-head"><div><span class="gbr-role">${unit.role}</span><b class="gbr-name">▰ ${unit.id}</b></div><span class="crew-state ${dispatched||unit.state!=='Свободен'?'busy':'free'} ${dispatched?progress:''}">${dispatched?progressLabel:unit.state}</span></div><div class="gbr-meta"><span>${unit.crew}</span><strong>${unit.distance}</strong><span>≈ ${unit.eta}</span></div>${dispatched?`<div class="gbr-progress ${progress}"><span class="gbr-progress-dot">●</span>${progressInfo}</div>`:''}<div class="gbr-actions"><button class="${dispatched?'cancel-gbr':'dispatch-gbr'}" data-unit="${unit.id}" ${!dispatched&&(blocked||unit.state!=='Свободен')?'disabled':''} title="${blocked?'Сначала примите тревогу в работу':''}">${dispatched?'Отменить ГБР':'Отправить'}</button><a class="icon-button phone" data-call-gbr="${unit.id}" href="tel:${unit.phone}" aria-label="Позвонить ${unit.id}">☎</a></div></article>`;}).join('');
...
function addHistoryForAlarm(alarm,event,details='',operator=currentOperator){const now=new Date();history.unshift({alarmNumber:alarm.number,tone:'red',date:now.toISOString().slice(0,10),time:now.toLocaleTimeString('ru-RU',{hour:'2-digit',minute:'2-digit',second:'2-digit'}),event,operator,details});if(selected?.number===alarm.number)renderHistory();}
function clearGbrProgressTimers(unit){if(unit.acceptTimer)clearTimeout(unit.acceptTimer);if(unit.arrivalTimer)clearTimeout(unit.arrivalTimer);unit.acceptTimer=null;unit.arrivalTimer=null;}
function scheduleGbrProgress(alarm,unit){
  clearGbrProgressTimers(unit);unit.dispatchProgress='sent';unit.acceptedAt='';unit.arrivedAt='';
  unit.acceptTimer=setTimeout(()=>{if(!alarms.includes(alarm)||!alarm.dispatchedGbr?.includes(unit.id))return;unit.dispatchProgress='accepted';unit.state='Принял тревогу';unit.acceptedAt=new Date().toLocaleTimeString('ru-RU',{hour:'2-digit',minute:'2-digit',second:'2-digit'});addHistoryForAlarm(alarm,'ГБР приняла тревогу',`${unit.id} • экипаж ${unit.crew}`,unit.id);if(selected?.number===alarm.number){renderGbr();toast(`${unit.id}: экипаж принял тревогу`);}unit.arrivalTimer=setTimeout(()=>{if(!alarms.includes(alarm)||!alarm.dispatchedGbr?.includes(unit.id))return;unit.dispatchProgress='arrived';unit.state='Прибыл на объект';unit.arrivedAt=new Date().toLocaleTimeString('ru-RU',{hour:'2-digit',minute:'2-digit',second:'2-digit'});addHistoryForAlarm(alarm,'ГБР прибыла на объект',`${unit.id} • экипаж ${unit.crew}`,unit.id);if(selected?.number===alarm.number){renderGbr();toast(`${unit.id}: экипаж прибыл на объект`);}},10*60*1000);},10000);
}
function dispatchGbr(unitId) {
  if(!requireWorkingShift())return;
  ...
  const alarm=selected,unit=getGbrUnits(alarm).find(item=>item.id===unitId);unit.state='Направлена';scheduleGbrProgress(alarm,unit);addHistory('ГБР направлена',`${unitId} • экипаж ${unit.crew} • ${unit.distance} • прибытие ${unit.eta}`);renderRows();renderDetail();toast(`${unitId} направлена на объект`);
}
function cancelGbr(unitId){
  if(!requireWorkingShift())return;
  selected.dispatchedGbr=(selected.dispatchedGbr||[]).filter(id=>id!==unitId);const unit=getGbrUnits(selected).find(item=>item.id===unitId);clearGbrProgressTimers(unit);unit.dispatchProgress='';unit.acceptedAt='';unit.arrivedAt='';unit.state='Свободен';addHistory('Выезд ГБР отменён',`${unitId} • экипаж ${unit.crew}`);
...
function getResponsibleContacts(alarm){
  ...
  const objectIndex=Math.max(0,alarms.indexOf(alarm));
  alarm.responsibleContacts=contactRoles.map((role,index)=>{const personIndex=objectIndex*contactRoles.length+index,surname=contactSurnames[personIndex%contactSurnames.length],name=contactNames[Math.floor(personIndex/contactSurnames.length)%contactNames.length],middleInitial=String.fromCharCode(1040+(seed+index*9)%32),digits=String((seed*7919+index*13729)%10000000).padStart(7,'0'),area=900+(seed+index*13)%90,phone=`+7${area}${digits}`,formatted=`+7 (${area}) ${digits.slice(0,3)}-${digits.slice(3,5)}-${digits.slice(5)}`;return [`${surname} ${name} ${middleInitial}.`,role,phone,formatted,personIndex%8];});
  return alarm.responsibleContacts;
}
function renderContacts(){const contacts=getResponsibleContacts(selected);$('contactsList').innerHTML=contacts.slice(0,2).map(c=>`<div class="contact"><span class="responsible-avatar avatar-color-${c[4]}" aria-hidden="true">${c[0].split(' ').map(s=>s[0]).join('').slice(0,2)}</span><div><b>${c[0]}</b><span>${c[1]}</span><strong>${c[3]}</strong></div><a class="icon-button phone" data-call-name="${c[0]}" href="tel:${c[2]}" aria-label="Позвонить ${c[0]}">☎</a></div>`).join('');}
...
function canHandleAlarms(){return shiftMode==='work';}
function shiftLockText(){if(shiftMode==='break')return 'Во время перерыва обработка тревог недоступна. Завершите перерыв, чтобы продолжить.';if(shiftMode==='lunch')return 'Во время обеда обработка тревог недоступна. Завершите обед, чтобы продолжить.';return shiftFinished?'Смена завершена. Начните новую смену, чтобы отрабатывать тревоги.':'Чтобы отрабатывать тревоги, нажмите «Начать смену».';}
function requireWorkingShift(){if(canHandleAlarms())return true;toast(shiftLockText());return false;}
function applyAlarmActionAvailability(){
  const locked=!canHandleAlarms(),message=shiftLockText(),lockMessage=$('shiftLockMessage');
  lockMessage.hidden=!locked;lockMessage.textContent=message;
  document.body.classList.toggle('alarm-actions-locked',locked);
  document.querySelectorAll('.take-alarm-row,#acceptBtn,#takeBtn,#finishBtn,#operatorCommentBtn,.dispatch-gbr,.cancel-gbr,#openCallCenter,#startCallCenter').forEach(control=>{control.disabled=locked;control.setAttribute('aria-disabled',String(locked));if(locked)control.title=message;else if(control.title===message||control.dataset.shiftLockedTitle==='true')control.removeAttribute('title');control.dataset.shiftLockedTitle=String(locked);});
  document.querySelectorAll('a.phone').forEach(link=>{link.classList.toggle('shift-disabled',locked);link.setAttribute('aria-disabled',String(locked));link.tabIndex=locked?-1:0;if(locked)link.title=message;else if(link.dataset.shiftLockedTitle==='true')link.removeAttribute('title');link.dataset.shiftLockedTitle=String(locked);});
}
function setShiftMode(mode){
  ...
  document.querySelector('.current-user small').textContent=active?labels[mode]:'Оператор'; renderShiftTimer(); renderOperatorList(); renderRows(); if(selected)renderDetail(); applyAlarmActionAvailability(); toast(labels[mode]);
}
...
function acceptAlarm(alarm){if(!requireWorkingShift()||!alarm||alarm.status!=='new')return;selected=alarm;selected.status='mine';selected.statusLabel='В РАБОТЕ';selected.operator=currentOperator;addHistory('Тревога принята в работу','Оператор назначен ответственным');renderRows();renderDetail();toast('Тревога принята в работу');}
$('acceptBtn').addEventListener('click',()=>acceptAlarm(selected));
$('takeBtn').addEventListener('click',()=>{if(!requireWorkingShift())return;$('takeObjectLabel').textContent=`Объект ${selected.number} • ${selected.name}`;$('takeOperatorLabel').textContent=`Текущий оператор: ${selected.operator}`;$('takeComment').value='';$('takeAlarmDialog').showModal();});
$('cancelTakeBtn').addEventListener('click',()=>$('takeAlarmDialog').close());
$('confirmTakeBtn').addEventListener('click',()=>{if(!requireWorkingShift())return;const comment=$('takeComment').value.trim();if(!comment){$('takeComment').focus();toast('Добавьте комментарий оператора');return;}const previous=selected.operator;selected.status='mine';selected.statusLabel='В РАБОТЕ';selected.operator=currentOperator;selected.transferComment=comment;addHistory('Тревога передана другому оператору',`От ${previous} к ${currentOperator} • Комментарий: ${comment}`);$('takeAlarmDialog').close();renderRows();renderDetail();toast('Тревога передана вам');});
function clearFinishChoices(){document.querySelectorAll('input[name="finishAction"],input[name="alarmResult"],input[name="dispatchType"]').forEach(input=>{input.checked=false;});}
$('finishBtn').addEventListener('click',()=>{if(!requireWorkingShift())return;clearFinishChoices();$('finishObjectLabel').textContent=`Объект ${selected.number} • ${selected.name}`;$('finishNote').value='';$('finishAlarmKind').value='';$('finishDispatchReason').value='';$('finishDispatchReason').disabled=false;document.querySelectorAll('input[name="dispatchType"]').forEach(input=>{input.disabled=false;});$('createServiceRequest').checked=false;$('serviceRequestFields').hidden=true;$('serviceRequestDescription').value='';$('serviceRequestPriority').value='Низкий';$('finishSystemType').value=isKts(selected)?'КТС':/пожар|пс/i.test(`${selected.event} ${selected.services}`)?'ПС':'ОС';$('finishDialog').showModal();});
...
$('confirmFinishBtn').addEventListener('click',()=>{
  if(!requireWorkingShift())return;
  const note=$('finishNote').value.trim()||'Комментарий не добавлен';
  const actionChoice=document.querySelector('input[name="finishAction"]:checked'),resultChoice=document.querySelector('input[name="alarmResult"]:checked'),dispatchChoice=document.querySelector('input[name="dispatchType"]:checked');
  if(!actionChoice){toast('Выберите действие: завершить или отменить');return;}
  if(!resultChoice){toast('Выберите результат тревоги');return;}
  const action=actionChoice.value,result=resultChoice.value,dispatchType=dispatchChoice?.value||'Не выбрана',alarmKind=$('finishAlarmKind').value||selected.event,dispatchReason=$('finishDispatchReason').value||'Не указана',createRequest=$('createServiceRequest').checked,requestDescription=$('serviceRequestDescription').value.trim(),requestPriority=$('serviceRequestPriority').value;
...
$('allContactsBtn').addEventListener('click',()=>{const contacts=getResponsibleContacts(selected);$('contactsObjectLabel').textContent=`Объект ${selected.number} • ${selected.name}`;$('contactsDialogBody').innerHTML=`<div class="contacts-toolbar"><button type="button" id="openCallCenter"><span class="headset-icon" aria-hidden="true"></span>Колл-центр</button></div><table class="events-full-table contacts-full-table"><thead><tr><th>№</th><th>ФИО</th><th>Роль</th><th>Телефон</th><th>Действия</th></tr></thead><tbody>${contacts.map((c,index)=>`<tr><td>${index+1}</td><td><b>${c[0]}</b></td><td>${c[1]}</td><td>${c[3]}</td><td><div class="contact-modal-actions"><a class="contact-call phone" data-call-name="${c[0]}" href="tel:${c[2]}" aria-label="Позвонить ${c[0]}" title="Позвонить">☎</a><button class="copy-phone" data-phone="${c[3]}" aria-label="Копировать номер ${c[0]}">⧉ Копировать</button></div></td></tr>`).join('')}</tbody></table>`;document.querySelectorAll('.copy-phone').forEach(button=>button.addEventListener('click',()=>copyText(button.dataset.phone,'Номер телефона скопирован')));$('openCallCenter').addEventListener('click',openCallCenter);$('contactsDialog').showModal();});
function openCallCenter(){if(!requireWorkingShift())return;$('contactsDialog').close();activeCallCenterContacts=getResponsibleContacts(selected);$('callCenterObjectLabel').textContent=`Объект ${selected.number} • ${selected.name}`;$('callCenterObject').value=`${selected.number}/1 — ${selected.name}`;$('callCenterTemplate').value='';$('callCenterSelectAll').checked=false;$('callResultsPanel').hidden=true;$('callCenterContacts').innerHTML=activeCallCenterContacts.map((contact,index)=>`<label><input type="checkbox" class="call-recipient" value="${index}"> <span><b>${contact[0]}</b><small>${contact[3]}</small></span></label>`).join('');$('startCallCenter').textContent='Запустить звонок';$('callCenterDialog').showModal();applyAlarmActionAvailability();}
...
$('startCallCenter').addEventListener('click',()=>{if(!requireWorkingShift())return;const recipientIndexes=[...document.querySelectorAll('.call-recipient:checked')].map(input=>Number(input.value));const template=$('callCenterTemplate').value;if(!recipientIndexes.length){toast('Выберите хотя бы одно ответственное лицо');return;}if(!template){toast('Выберите шаблон звонка');return;}const primaryType=callResultTypeForTemplate(template);const results=recipientIndexes.map((index,position)=>{const contact=activeCallCenterContacts[index];return{name:contact[0],phone:contact[3],type:position===0?primaryType:'noAnswer'};}).sort((a,b)=>Number(callResultTypes[b.type].answered)-Number(callResultTypes[a.type].answered));selected.callResults=results;const primary=callResultTypes[results[0].type];$('callResultMainIcon').src=primary.icon;$('callResultMainIcon').alt=primary.label;$('callResultInstruction').innerHTML=`<strong>${primary.label}</strong><span>${primary.instruction}</span>`;$('callResultsList').innerHTML=results.map(result=>{const type=callResultTypes[result.type];return`<article class="call-result-row ${type.answered?'answered':'missed'}"><img src="${type.icon}" alt=""><div><b>${result.name}</b><span>${result.phone}</span><small>${type.label}</small></div></article>`;}).join('');$('callResultsPanel').hidden=false;$('startCallCenter').textContent='Повторить звонок';addHistory('Результат обзвона',`${primary.label} • ${results[0].name} • ${results[0].phone}`);renderRows();toast(`Результат: ${primary.label}`);});
...
function renderReports(reports=completedReports){
  $('reportsBody').innerHTML=reports.length?reports.map((report,index)=>`<article class="report-card"><div class="report-summary"><div><b>Объект ${report.number} • ${report.name}</b><span>${report.address}</span></div><div><span>Завершено</span><strong>${report.completedAt}</strong></div><div><span>Оператор</span><strong>${report.operator}</strong></div></div><details><summary>Отработка тревоги и аналитика</summary><div class="report-details"><dl><div><dt>Событие</dt><dd>${report.event}</dd></div><div><dt>Категория</dt><dd>${report.category||2}</dd></div><div><dt>Результат</dt><dd>${report.result}</dd></div><div><dt>ГБР</dt><dd>${report.gbr}</dd></div><div><dt>Комментарий оператора</dt><dd>${report.operatorComment||'Комментарий не добавлен'}</dd></div><div><dt>Пульт</dt><dd>${report.panel||'—'}</dd></div><div><dt>Раздел</dt><dd>${report.section||'—'}</dd></div><div><dt>Администратор</dt><dd>${report.administrator}</dd></div><div><dt>Менеджер</dt><dd>${report.manager}</dd></div><div><dt>Событий в хронологии</dt><dd>${report.chronology.length}</dd></div></dl><div class="report-history-head"><h3>Хронология отработки</h3><button class="copy-report-history" data-report-index="${index}">⧉ Копировать хронологию</button></div><table><thead><tr><th>Время</th><th>Действие</th><th>Оператор</th><th>Детали</th></tr></thead><tbody>${sortHistory(report.chronology,'asc').map(item=>`<tr><td>${item.time}</td><td>${item.event}</td><td>${item.operator}</td><td>${item.details||'—'}</td></tr>`).join('')}</tbody></table></div></details></article>`).join(''):'<div class="events-empty">По заданным условиям отчёты не найдены</div>';
...
function reportIsoDate(report){const match=report.completedAt.match(/^(\d{2})\.(\d{2})\.(\d{4})/);return match?`${match[3]}-${match[2]}-${match[1]}`:'';}
function getFilteredReports(){const from=$('reportsDateFrom').value,to=$('reportsDateTo').value,number=$('reportsNumberSearch').value.trim().toLowerCase();return completedReports.filter(report=>{const date=reportIsoDate(report);return(!from||date>=from)&&(!to||date<=to)&&(!number||report.number.toLowerCase().includes(number));});}
function applyReportsFilter(){const reports=getFilteredReports();renderReports(reports);return reports;}
function xmlEscape(value){return String(value??'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&apos;');}
function exportReportsToExcel(){
  const reports=getFilteredReports();
  if(!reports.length){toast('Нет отчётов для выгрузки');return;}
  const cell=(value,style='Data')=>`<Cell ss:StyleID="${style}"><Data ss:Type="String">${xmlEscape(value)}</Data></Cell>`;
  const row=(cells,style='Data',height='22')=>`<Row ss:Height="${height}">${cells.map(value=>cell(value,style)).join('')}</Row>`;
  const rows=[];
  rows.push(`<Row ss:Height="34"><Cell ss:StyleID="ReportTitle" ss:MergeAcross="3"><Data ss:Type="String">ОТЧЁТЫ ПО ЗАВЕРШЁННЫМ ТРЕВОГАМ</Data></Cell></Row>`);
  rows.push(`<Row ss:Height="22"><Cell ss:StyleID="ReportSubtitle" ss:MergeAcross="3"><Data ss:Type="String">Сформировано ${xmlEscape(new Date().toLocaleString('ru-RU'))}</Data></Cell></Row>`);
  reports.forEach((report,index)=>{
    if(index)rows.push(row(['','','',''],'Spacer','12'));
    rows.push(`<Row ss:Height="30"><Cell ss:StyleID="CardTitle" ss:MergeAcross="3"><Data ss:Type="String">Объект ${xmlEscape(report.number)} • ${xmlEscape(report.name)}</Data></Cell></Row>`);
    const details=[['Адрес',report.address],['Пультовый номер',report.number],['Пульт',report.panel||'—'],['Раздел',report.section||'—'],['Категория',report.category||2],['Событие',report.event],['Завершено',report.completedAt],['Оператор',report.operator],['Результат',report.result],['Комментарий оператора',report.operatorComment||'Комментарий не добавлен'],['ГБР',report.gbr],['Администратор',report.administrator],['Менеджер',report.manager]];
    details.forEach(([label,value])=>rows.push(`<Row ss:AutoFitHeight="1"><Cell ss:StyleID="Label"><Data ss:Type="String">${xmlEscape(label)}</Data></Cell><Cell ss:StyleID="Value" ss:MergeAcross="2"><Data ss:Type="String">${xmlEscape(value)}</Data></Cell></Row>`));
    rows.push(`<Row ss:Height="28"><Cell ss:StyleID="SectionTitle" ss:MergeAcross="3"><Data ss:Type="String">Хронология отработки</Data></Cell></Row>`);
    rows.push(row(['Время','Действие','Оператор','Детали'],'TableHeader','26'));
    sortHistory(report.chronology,'asc').forEach(item=>rows.push(row([item.time,item.event,item.operator,item.details||'—'],'Data','24')));
  });
  const styles=`<Styles><Style ss:ID="Default" ss:Name="Normal"><Alignment ss:Vertical="Center"/><Font ss:FontName="Inter" ss:Size="10" ss:Color="#182128"/></Style><Style ss:ID="ReportTitle"><Alignment ss:Horizontal="Left" ss:Vertical="Center"/><Font ss:FontName="Inter" ss:Size="16" ss:Bold="1" ss:Color="#FFFFFF"/><Interior ss:Color="#103A5A" ss:Pattern="Solid"/></Style><Style ss:ID="ReportSubtitle"><Alignment ss:Vertical="Center"/><Font ss:FontName="Inter" ss:Size="9" ss:Color="#D4E8F8"/><Interior ss:Color="#103A5A" ss:Pattern="Solid"/></Style><Style ss:ID="CardTitle"><Alignment ss:Vertical="Center"/><Font ss:FontName="Inter" ss:Size="12" ss:Bold="1" ss:Color="#FFFFFF"/><Interior ss:Color="#174C6B" ss:Pattern="Solid"/><Borders><Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#8FC9F2"/></Borders></Style><Style ss:ID="Label"><Alignment ss:Vertical="Top" ss:WrapText="1"/><Font ss:FontName="Inter" ss:Size="10" ss:Bold="1" ss:Color="#082F49"/><Interior ss:Color="#D4E8F8" ss:Pattern="Solid"/><Borders><Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#C8D3DA"/><Border ss:Position="Right" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#C8D3DA"/></Borders></Style><Style ss:ID="Value"><Alignment ss:Vertical="Top" ss:WrapText="1"/><Font ss:FontName="Inter" ss:Size="10" ss:Color="#182128"/><Interior ss:Color="#FFFFFF" ss:Pattern="Solid"/><Borders><Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#C8D3DA"/></Borders></Style><Style ss:ID="SectionTitle"><Alignment ss:Vertical="Center"/><Font ss:FontName="Inter" ss:Size="11" ss:Bold="1" ss:Color="#FFFFFF"/><Interior ss:Color="#174C6B" ss:Pattern="Solid"/></Style><Style ss:ID="TableHeader"><Alignment ss:Horizontal="Left" ss:Vertical="Center" ss:WrapText="1"/><Font ss:FontName="Inter" ss:Size="10" ss:Bold="1" ss:Color="#FFFFFF"/><Interior ss:Color="#223039" ss:Pattern="Solid"/><Borders><Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#84949F"/><Border ss:Position="Right" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#84949F"/></Borders></Style><Style ss:ID="Data"><Alignment ss:Vertical="Top" ss:WrapText="1"/><Font ss:FontName="Inter" ss:Size="10" ss:Color="#182128"/><Interior ss:Color="#F8FAFC" ss:Pattern="Solid"/><Borders><Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#C8D3DA"/><Border ss:Position="Right" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#C8D3DA"/></Borders></Style><Style ss:ID="Spacer"><Interior ss:Color="#EEF3F6" ss:Pattern="Solid"/></Style></Styles>`;
  const xml=`<?xml version="1.0"?><?mso-application progid="Excel.Sheet"?><Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet">${styles}<Worksheet ss:Name="Завершённые тревоги"><Table ss:DefaultRowHeight="22"><Column ss:Width="125"/><Column ss:Width="230"/><Column ss:Width="130"/><Column ss:Width="260"/>${rows.join('')}</Table><WorksheetOptions xmlns="urn:schemas-microsoft-com:office:excel"><FreezePanes/><FrozenNoSplit/><SplitHorizontal>2</SplitHorizontal><TopRowBottomPane>2</TopRowBottomPane><ActivePane>2</ActivePane><ProtectObjects>False</ProtectObjects><ProtectScenarios>False</ProtectScenarios></WorksheetOptions></Worksheet></Workbook>`;
  const url=URL.createObjectURL(new Blob(['\ufeff',xml],{type:'application/vnd.ms-excel;charset=utf-8'}));
  const link=document.createElement('a');link.href=url;link.download=`Отчёты_по_тревогам_${new Date().toISOString().slice(0,10)}.xls`;link.click();setTimeout(()=>URL.revokeObjectURL(url),1000);toast('Оформленный отчёт Excel сформирован');
}
...
$('applyReportsFilter').addEventListener('click',applyReportsFilter);
$('resetReportsFilter').addEventListener('click',()=>{$('reportsDateFrom').value='';$('reportsDateTo').value='';$('reportsNumberSearch').value='';renderReports();});
$('reportsNumberSearch').addEventListener('input',applyReportsFilter);
$('exportReportsExcel').addEventListener('click',exportReportsToExcel);
